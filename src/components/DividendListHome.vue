                ...
                <table class="table table-striped table-hover align-middle">
                    ...
                    <tbody>
                        <tr v-for="item in stocks" v-bind:key="item._id" class='clickable-row' v-on:click="goToStock(item.ticker)">
                            ...
                            <td>{{item.ticker}}</td>
                            <td>{{item.name}}</td>
                            <td>{{item.dividendyieldnice}}</td>
                            <td>{{item.payoutrationice}}</td>
                            <td>{{item.freecashflownice}}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
...
export default {
    ...
    data: function() {
        return {
            stocks: [],
            cardTitle: "First 10 stocks by dividend yield"
        }
    },
    async mounted() {
        await axios.get(`${process.env.VUE_APP_STOCKSCLIENTAPI}?select=name,ticker,dividendyieldnice,payoutrationice,freecashflownice&sort=-dividendyield&limit=10`)
